//
//  SendTokenModalViewController.swift
//  KlipSDK-Sample
//
...
class SendTokenModalViewController: UIViewController, UITextFieldDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SendTokenError" {
    ...
    @objc func keyboardWillShow(_ sender: Notification) {
        if let keyboardFrame: NSValue = sender.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue {
            ...
            if keyboardHeight == 0.0 || keyboardShown == true || self.toAddress.isTracking || self.contractAddress.isTracking || self.amount.isTracking || self.fromAddress.isTracking {
                return
            }
    ...
    @IBOutlet weak var SendTokenCancel: UIButton!
    @IBAction func hideSendTokenModal(_ sender: Any) {
    ...
    @IBOutlet weak var amount: UITextField! {didSet {amount.delegate = self}}
    ...
    @IBAction func prepareSendToken(_ sender: Any) {
        if (bappName.text?.isEmpty == false && contractAddress.text?.isEmpty == false && toAddress.text?.isEmpty == false && amount.text?.isEmpty == false && amount.text != "0") {
            let tokenTxRequest = TokenTxRequest(to: toAddress.text!, amount: amount.text!, contract: contractAddress.text!, from: fromAddress.text!)
            ...
            KlipSDK.shared.prepare(request: tokenTxRequest, bappInfo: bappInfo) { result in
                switch result {
                ...
                case .failure(let error):
                    ...
                    DispatchQueue.main.async {
                        self.performSegue(withIdentifier: "SendTokenError", sender: self)
                    }
                }
            }
        }
